...
export function useBoard() {
    ...
    const [position, setPosition] = useState({ x: Math.floor(COLUMN_COUNT / 2), y: 0 });
    ...
    useEffect(updateDisplay, [scene, shape, position,score]);
    useEffect(removeFullLines, [scene,score]);
    useInterval(tick, 600);

    function updateDisplay() {
        const newDisplay = mergeIntoStage(scene, shape, position,score);
    ...
    function placeShape() {
        ...
        if (position.y === 0 && !validPosition({ x: Math.floor(COLUMN_COUNT / 2), y: 0 }, shape)) {
            setGameOver(true);
        } else {
            if (!gameOver) {
                setShape(randomShape());
                setPosition({ x: Math.floor(COLUMN_COUNT / 2), y: 0 });
            }
        }
    }
    ...
    return [display, onKeyDown, gameOver];
}